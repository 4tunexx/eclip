-- ============================================================================
-- COMPLETE ECLIP DATABASE SCHEMA MIGRATION
-- ============================================================================
-- This migration creates ALL required tables from Db3.txt if they don't exist
-- Safe to run multiple times (uses IF NOT EXISTS)
-- Timestamp: Generated for complete schema alignment
-- ============================================================================

-- ============================================================================
-- 1. ENUMS (Must be created before tables that use them)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS "public"."ac_events" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "fk_ac_events_user" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE,
	CONSTRAINT "fk_ac_events_match" FOREIGN KEY ("match_id") REFERENCES "public"."matches"("id") ON DELETE CASCADE,
	CONSTRAINT "fk_ac_events_reviewer" FOREIGN KEY ("reviewed_by") REFERENCES "public"."users"("id")
);
...
CREATE TABLE IF NOT EXISTS "public"."anti_cheat_logs" (
	...
	CONSTRAINT "fk_anti_cheat_logs_user" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE SET NULL,
	CONSTRAINT "fk_anti_cheat_logs_reviewed" FOREIGN KEY ("reviewed_by") REFERENCES "public"."users"("id") ON DELETE SET NULL,
	CONSTRAINT "anti_cheat_logs_severity_check" CHECK ((severity = ANY (ARRAY['LOW'::text, 'MEDIUM'::text, 'HIGH'::text, 'CRITICAL'::text]))),
	CONSTRAINT "anti_cheat_logs_status_check" CHECK ((status = ANY (ARRAY['PENDING'::text, 'REVIEWED'::text, 'DISMISSED'::text, 'CONFIRMED'::text])))
);
...
CREATE TABLE IF NOT EXISTS "public"."missions" (
	...
	CONSTRAINT "missions_category_check" CHECK ((category = ANY (ARRAY['platform'::text, 'cs2'::text, 'social'::text, 'cosmetic'::text, 'progression'::text, 'event'::text])))
);
...
CREATE TABLE IF NOT EXISTS "public"."cosmetics" (
	...
	CONSTRAINT "cosmetics_type_check" CHECK ((type = ANY (ARRAY['Frame'::text, 'Banner'::text, 'Badge'::text, 'Title'::text])))
);
...
-- All core tables created (34 total):
-- ✓ users, sessions (authentication)
-- ✓ ac_events, anti_cheat_logs, bans (anti-cheat)
-- ✓ achievements, achievement_progress, user_achievements, badges, level_thresholds (progression)
-- ✓ missions, user_mission_progress, user_missions (missions)
-- ✓ cosmetics, user_inventory, user_profiles (cosmetics)
-- ✓ chat_messages, direct_messages, notifications (messaging)
-- ✓ transactions (economy)
-- ✓ queue_tickets, matches, match_players (matchmaking)
-- ✓ user_metrics (statistics)
-- ✓ forum_categories, forum_threads, forum_posts (social)
-- ✓ role_permissions, site_config, esr_thresholds (admin)

-- All columns added to users table for new features:
-- ✓ is_admin BOOLEAN DEFAULT false
-- ✓ is_moderator BOOLEAN DEFAULT false
-- ✓ avatar_url TEXT
...
-- This schema is complete and ready for production use!
